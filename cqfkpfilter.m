function [ xt_filter, fk_filter, fk, f, k ] = ...
    cqfkpfilter( xt, dt, dx,us, f0, f1, tswp, phi_f,pshift_x0,pmin, pmax  )
...
% phi_f = (optional) if it is provided then the program will not use any analytical
%   equation but the numerical interpolation to calculate phase-correction filter
%       phi_f{1} = normalized numerical calculated phase distortion vector
%       phi_f{2} = frequency vector for the normalized phase distortion vector
...
% check to see if to use analytical or numerical phase distortion function
if ~exist('phi_f','var') || isempty(phi_f)
    phase_correction = -2 * pi * doppler_factor * tswp .* f_mat.* (f_mat-f0) ./ (f1 - f0);
else
    phi = phi_f{1}; f_phi = phi_f{2}; 
    phi = phi(:); f_phi = f_phi(:);
    phi_interp = interp1(f_phi,phi,f_mat(:,1),'spline',0);
    phi_interp = repmat(phi_interp,1,nx);
    phase_correction = doppler_factor .* -phi_interp;
end
